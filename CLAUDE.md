...
- **Adding a new role**: Create `roles/<rolename>/` with a `setup` script and/or `zsh_plugin` file. Add a `requires` file listing dependency roles if needed. Add the role name to relevant host role files.
